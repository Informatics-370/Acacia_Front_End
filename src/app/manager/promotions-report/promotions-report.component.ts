...
@Component({
  ...
  styleUrls: ['./promotions-report.component.scss']
})
export class PromotionsReportComponent {
  ...
  loadReport() {
    this.reportService.getPromotionsReport(this.salesParams).subscribe({
      next: report => {
        this.Report = report;
  ...
  public printPdf(): void {
    ...
    // Add report title
    PDF.setFontSize(18);
    PDF.text('PROMOTIONS REPORT', fileWidth / 20, 55, { align: 'left' });
